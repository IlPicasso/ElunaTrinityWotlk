...
class TC_GAME_API ThreatManager
{
    public:
        ...
        Trinity::Containers::IteratorPair<threat_list_heap::const_iterator> GetUnsortedThreatList() const { return { _sortedThreatList.begin(), _sortedThreatList.end() }; }
        ...
        Trinity::Containers::IteratorPair<threat_list_heap::ordered_iterator> GetSortedThreatList() const { return { _sortedThreatList.ordered_begin(), _sortedThreatList.ordered_end() }; }
        // slowest of the three threat list getters (by far), but lets you modify the threat references
        std::list<ThreatReference*> GetModifiableThreatList() const;
        ...
        std::unordered_map<ObjectGuid, ThreatReference*> const& GetThreatenedByMeList() const { return _threatenedByMe; }
        ...
        // What it says on the tin - modify target's threat by +percent%
    ...
    private:
        ...
        std::list<std::pair<ObjectGuid, uint32>> _redirectInfo; // current redirection targets and percentages (updated from registry in ThreatManager::UpdateRedirectInfo)
